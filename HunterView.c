...
/**
 * Performs a BFS for the given dracula starting at `src`, assuming the
 * round is `r`. Returns a predecessor array.
 */
PlaceId *draculaBfs(HunterView hv, PlaceId src, Round r)
{
	PlaceId *pred = malloc(NUM_REAL_PLACES * sizeof(PlaceId));
	placesFill(pred, NUM_REAL_PLACES, -1);
	pred[src] = src;
	...
	Queue q1 = QueueNew(); // current link locations
	Queue q2 = QueueNew(); // next link locations
	
	QueueEnqueue(q1, src);
	while (!(QueueIsEmpty(q1) && QueueIsEmpty(q2)) && r > 0) {
		PlaceId curr = QueueDequeue(q1);
		int numReachable = 0;
		PlaceId *reachable = GvGetReachableByType(hv->gv, PLAYER_DRACULA, r, curr,
		...
		// When we've exhausted the current link's locations, advance
		// to the next link and swap the queues (so the next link's
		// locations becomes the current link's locations)
		if (QueueIsEmpty(q1)) {
			Queue tmp = q1; q1 = q2; q2 = tmp; // swap queues
			r--;
		}
...
PlaceId *HvGetDraculaLocations(HunterView hv, PlaceId src, Round r,
                                                           int *numReturnedLocs)
{
	PlaceId *pred = draculaBfs(hv, src, r);
	
	// One pass to get the location size
	int size = 0;
	...
	PlaceId *locations = malloc(size * sizeof(PlaceId));
	// Another pass to copy the locations
   for (int i = 0, j = 0; i < NUM_REAL_PLACES; i++) {
      if (pred[i] != -1) {
         locations[j] = i;
         j++;
      }
...
int HvFillDraculaLocations(HunterView hv, PlaceId locations[], Round r[], int size)
{
   ...
   bool canFree = false; 
   PlaceId *history = GvGetLocationHistory(hv->gv, PLAYER_DRACULA,
